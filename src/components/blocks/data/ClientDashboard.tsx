...
const ClientDashboard = () => {
	...
	return dbView ? (
		<div className="p-2">
			...
			<hr />
			<div className="">
				# of messages: {dbView.numMessages + dbView.numMessagesMarkedForPruning}
			</div>
			<div className="">
				&nbsp;&nbsp;&nbsp;&bull;&nbsp;valid: {dbView.numMessages}
			</div>
			<div className="">
				&nbsp;&nbsp;&nbsp;&bull;&nbsp;marked for pruning:{" "}
				{dbView.numMessagesMarkedForPruning}
			</div>
			<div className=""># of unique users (FIDs): {dbView.numFids}</div>
			<span className="text-emphasis text-xs">
				as of {new Date().toLocaleDateString().split(" ")[0]}
			</span>
		</div>
	) : null;
};
